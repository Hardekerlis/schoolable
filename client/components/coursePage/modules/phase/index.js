...
import HandInMenu from './handInMenu';
...
const Phase = ({ data, className }) => {
  ...
  return(
    <div ref={phaseRef} className={(handInMenuOpen) ? `${phaseClassName} ${styles.handInMenuOpen}` : phaseClassName}>
      <div className={styles.container}>
        <div className={styles.header}>
          <p className={styles.title}>{firstLetterToUpperCase("" + data?.name)}</p>
          {canHandIn &&
            <div onClick={openHandInMenu} className={styles.handIn}>
              <p className={styles.text}>{(handInMenuOpen) ? 'Close' : 'Hand in'}</p>
            </div>
          }
        </div>
