...
const CourseCreation = ({ fetchCourses, currentCourses, setCurrentCourses }) => {
  ...
  const courseCreationSubmit = async(evt) => {
    ...
    let request = new Request('/api/course/create', { name: newCourseName }).post().json();
    ...
    <>
      ...
      { currentCourses.length === 0 ?
        <>
          ...
          <div className={styles.createFirstCourseWrapper}>
            ...
            <p className={styles.helperText}>You currently don’t own any courses. <br /> Create one to start!</p>
      ...
      <div ref={courseCreationRef} className={styles.newCourseCreation}>
        <form onSubmit={courseCreationSubmit}>
          <input onChange={newCourseNameChange} value={newCourseName} autoFocus placeholder="Course name" />
          <div className={styles.buttonContainer}>
            <button type="button" onClick={closeCourseCreation}>Exit</button>
