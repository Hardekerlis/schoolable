...
const Calendar = ({ sType }) => {
  ...
  const generateSchedule = () => {
    ...
    switch (scheduleType) {
      ...
      case 'timeline':
        setCompleteRender(
          <Timeline data={sevenDayData} date={selectedDayObject} />,
        );
        break;
      case 'default':
        break;
  ...
  const nav = dir => {
    ...
    if(scheduleType === 'week') {
      ...
      setCurrentDay(currentDay.plus({ month: dir }));
      return;
    }

    setCurrentDay(currentDay.plus({ day: daysToAdd * dir }));
  };
  ...
  return (
    <Layout mainClass={styles.headWrapper}>
      <div
        className={isFullDesktop ? styles.wrapper : `${styles.wrapper} ${styles.mobile}`}
      >
        <Sidebar />
